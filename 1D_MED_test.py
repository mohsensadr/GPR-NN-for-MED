...
dimY = 4; dimX =dimY-2;
i0 = 2; i1=i0+dimX; i11=2*dimY; i2=i11+dimY;

'''
        ----    Reading Data      ----
'''
address = "data_test/"+str(dimY)+"_10_N_4000.txt"
...
np.savetxt("data_test/GPR_mean_input.dat",mm[i0:i1],delimiter=',')
np.savetxt('data_test/GPR_variance_input.dat',vvar[i0:i1],delimiter=',')

np.savetxt('data_test/GPR_mean_output.dat',mm[i11:i2],delimiter=',')
np.savetxt('data_test/GPR_variance_output.dat',vvar[i11:i2],delimiter=',')

x0 = transf_to(x0,mm,vvar,dim);
#x = Remove_Bad_Data(x0)
...
if method == "GPR":
    ...
    if with_mean == "yes":
        A = np.zeros((dimX, dimY))
        b = np.zeros((1, dimY))
        meanf = gp.mean_functions.Linear(A, b)
        model = gp.models.GPR(xtrain, ytrain, kernel, meanf)
        f_name = meth+"_withmean"
    else:
    ...
    ''' shared kernel in the output
    # create multioutput kernel
    kernel = mk.SharedIndependentMok(gp.kernels.RBF(input_dim=dimX, active_dims=len_sc, ARD=True), output_dimensionality=dimY)
    # create multioutput features from Z
    feature = mf.SharedIndependentMof(gp.features.InducingPoints(Z))
    f_name += "_SharKernShaFeat_"+str(N1)
    '''
    ...
    kern_list = [gp.kernels.Matern32(input_dim=dimX, active_dims=len_sc, ARD=True)  for _ in range(dimY)]
    # Create multioutput kernel from kernel list
    kernel = mk.SeparateIndependentMok(kern_list)
    ...
    model = gp.models.SVGP(xtrain, ytrain, kernel, gp.likelihoods.Gaussian(), feat=feature,  whiten=False)

    model.likelihood.variance = 1e-10
    model.likelihood.variance.trainable = False

    opt = gp.train.ScipyOptimizer(tol=1e-12, options={"eps": 1E-12, "disp": True, 'ftol': 1e-12})
    opt.minimize(model, disp=True, maxiter=100000)

'''
save the model 
'''
#import pickle
m_name = "data_test/"+str(dimY)+"l_"+str(N1)+".md"
